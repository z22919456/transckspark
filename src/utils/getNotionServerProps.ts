...
const getPageServerSideProps = (dbId: string) => async (context: GetStaticPropsContext<{ id: string }>) => {
  ...
  try {
    ...
    // 如果newSignedUrl是空的，代表沒有圖片
    if (Object.keys(newSignedUrl).length === 0) {
      const key = pageInformation?.id || '';
      const coverUrl = pageInformation?.cover.url || '';
      if (key && coverUrl) {
        newSignedUrl[key] = await uploadImage(coverUrl, 5);
        signedUrlKeys.push(key);
      }
    }
